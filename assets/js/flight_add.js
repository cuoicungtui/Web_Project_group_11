...
function check_flight_add(){
    ...
    let s = new Date(arr[1].value+" "+arr[3].value);
    let e = new Date(arr[2].value+" "+arr[4].value);
   ...
   // 
    ...
    {   alert("Start time cannot be greater than end date\n Start time > day now")
        arr[1].style.borderColor = "yellow";
        arr[2].style.borderColor = "yellow";
        arr[3].style.borderColor = "yellow";
        arr[4].style.borderColor = "yellow";
        return false;
    } 
    if(Number(arr[5].value) < Number( arr[6].value)  || Number(arr[6].value) <= 0 || Number(arr[5].value) <=0 ){
